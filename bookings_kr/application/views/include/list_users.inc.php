...
?>
	<form action="<?php echo base_url() ?>user_edit" id="form_edit" name="form_edit" method="post">
		<input type="hidden" id="user_type_form" name="user_type_form" value="<?php echo $user_level ?>" />
		...
		<table class="table">
			...
			?>
				<tr><?
					...
					<td><?php echo $special ?><?php echo $usr_name  ?></td>
					<td><?php echo $usr_surname ?></td>
					<td><?php echo $item->usr_phone_main ?></td>
					...
					?>
						...
						<td><?php echo $h_confirm; ?></td>
						<td><?php echo $h_booked; ?></td>
						<td><?php echo $h_penalty; ?></td>
						<td><?php echo $h_left; ?></td>
					<?php
					}
					if ($user_level == 3) {
					?><td>
							<a href="<?php echo base_url() ?>extract/<?php echo $item->usr_id ?>" target="_blank">
								<i class="icon-book"></i>
							</a>
						</td>
						<td>
							<a href="<?php echo base_url() ?>instructors/schedule/<?php echo $item->usr_id ?>">
								<i class="icon-list"></i>
							</a>
						</td>
					<?php
					...
					<td>
						<a class="data_edit" href="javascript:submit_form('<?php echo $item->usr_id ?>')"><i class="icon-pencil"></i></a>
						<a href="#myModal" role="button" data-toggle="modal" data-id="<?php echo $item->usr_id ?>" class="data_del"><i class="icon-remove"></i></a>
					</td>
				</tr>
			<?
			}
	...
	<div class="modal small hide fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		...
		<div class="modal-footer">
			<form method="post" action="<?php echo base_url() ?>welcome/delete_user">
				<input type="hidden" id="user_type_form" name="user_type_form" value="<?php echo $user_level ?>" />
	...
	<div class="pagination">
		...
		<div class="search_users">
			<div class="input-append">
				<form id="search_form" name="search_form" action="<?php echo base_url() . $value_search ?>" method="post	" />
				<input type="text" class="span2 search-query" name="search_user" id="search_user" value="<?php echo $this->input->post('search_user') ?>">
